...
def preprocess_shear(files,filemaskpercents,scalingStr,fileUgs=None,fileRes=None,fileB0s=None,size=1,irun=''):
    ...
    for ifile in range(len(files)): #tdqm notebook just draws progress bar
        ...
        outputFields = np.array([tau_12, tau_13, tau_23])
        ...
        if scalingStr=='local':
            ...
            tijScale=np.array([TKE,ti3Scale,ti3Scale])
...
def preprocess_full(files,filemaskpercents,scalingStr,fileUgs=None,fileRes=None,fileB0s=None,size=3,irun=''):
    # Any of the below could be changed to inputs of the function
    path="/burg/glab/users/ac5006/DNStoLES/coarseData/fullTau/"
    #size = 1 # number of neighboring points to include in each input sample
    size=int((size-1)/2)
...
class C4NN(torch.nn.Module):
    def __init__(self,Nhid,size):
        super(C4NN, self).__init__()
        ...
        self.feat_type_out = nn.FieldType(r2_act, [r2_act.irrep(2),r2_act.irrep(1)])
