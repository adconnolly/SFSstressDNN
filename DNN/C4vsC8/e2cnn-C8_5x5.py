...
sizeStr=str(size)
scaleStr='local'
saveFile='C8_full_'+sizeStr+'x'+sizeStr+'_'
...
for irun in range(Nruns):
    ...
    print("Train Files:")
    x_train, y_train, tauScale_train, trainMask = preprocess_full(files, filemaskpercents, scaleStr, fileUgs, fileRes, fileB0s, size, irun)
    ...
    print("Test Files:")
    xtest, ytest, tauScaletest, testMask = preprocess_full(testFiles, testfilemaskpercents, scaleStr, testfileUgs, testfileRes, testfileB0s,size,irun)
    ...
    model=CNNN(Nhid,N=8,size=size,device=device).float().to(device)
